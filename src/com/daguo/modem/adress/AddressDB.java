...
public class AddressDB extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase db) {
		String address = "create table " + MY_ADDRESS + "(" + ID
				+ " varchar(20) primary key ," + PROVINCES + " varchar(100),"
				+ STREET + " varchar(100)," + NAME + " varchar(20)," + PHONE
				+ " varchar(20)," + STATUS + " boolean)";
		db.execSQL(address);
	}
	...
	public boolean updeteAddress(AddressInfo address) {
		SQLiteDatabase db = myAddress.getWritableDatabase();
		ContentValues values = new ContentValues();
		values.put(ID, address.getId());
		values.put(PROVINCES, address.getProvinces());
		...
		values.put(PHONE, address.getPhone());
		values.put(STATUS, address.isStatus());
		long i = db.update(MY_ADDRESS, values, ID + "=?",
	...
	public List<AddressInfo> queryAddress() {
		...
		Cursor cursor = db.query(MY_ADDRESS, new String[] { ID, PROVINCES,
				STREET, NAME, PHONE, STATUS }, null, null, null, null, null);
	...
	public String CursorOneDate(String who) {
		...
		Cursor cursor = db.query(MY_ADDRESS, null, STATUS + "=?",
				new String[] { who }, null, null, null);

		if (cursor.getCount() > 0) {
			cursor.moveToFirst();

			messageString = cursor.getString(3) + "," + cursor.getString(4)
	...
	public List<AddressInfo> oneAddress(String who) {
		SQLiteDatabase db = myAddress.getWritableDatabase();
		List<AddressInfo> list = new ArrayList<AddressInfo>();
