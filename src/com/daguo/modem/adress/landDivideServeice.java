...
public class landDivideServeice extends IntentService {
	...
	protected void onHandleIntent(Intent intent) {
		LandDivideDB landDivideDB = LandDivideDB.getInstance(getBaseContext());
			...
			for (int i = 0; i < txt.length; i++) {
				...
				try {
					...
					for (int j = 0; j < jsonArray.length(); j++) {
						...
						landDivide.setSuperior(temp.getString("superior"));
			...
			for (int i = 0; i < txt.length; i++) {
				...
				try {
					...
					for (int j = 0; j < jsonArray.length(); j++) {
						...
						List<LandDivide> mList = landDivideDB.queryAddress(
								"code=?",
								new String[] { temp.getString("code") });
						if (mList == null) {
							landDivideDB.insertAddress(landDivide);
						}
					}
